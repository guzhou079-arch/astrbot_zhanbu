...
class DivinationPlugin(Star):
    ...
    async def tool_meihua(self, event: AstrMessageEvent, question: str, number1: int = 0, number2: int = 0) -> MessageEventResult:
        '''梅花易数占卜起卦。当用户想要占卜、算卦、算命、测运势、问吉凶时调用此工具。调用后你会收到完整卦象数据，请你务必先展示卦象，然后用你自己的人设风格对卦象进行详细解读（200-400字），包括本卦变卦含义、体用关系分析、对所问之事的建议，最后提醒这是娱乐性质的占卜。
        ...
        text += "\n\n【请你现在立刻用自己的人设风格，对以上卦象进行详细解读，分析本卦变卦含义和体用关系，给出对所问之事的建议，最后提醒是娱乐占卜。不要只展示卦象不解读。】"
    ...
    async def tool_liuyao(self, event: AstrMessageEvent, question: str) -> MessageEventResult:
        '''六爻铜钱摇卦占卜。当用户明确要求用六爻、铜钱、摇卦方式占卜时调用此工具。调用后你会收到完整卦象数据，请你务必先展示卦象，然后用你自己的人设风格对卦象进行详细解读（200-400字），包括本卦变卦含义、动爻分析、对所问之事的建议，最后提醒这是娱乐性质的占卜。
        ...
        text = result.format_text()
        text += "\n\n【请你现在立刻用自己的人设风格，对以上卦象进行详细解读，分析本卦变卦含义和动爻，给出对所问之事的建议，最后提醒是娱乐占卜。不要只展示卦象不解读。】"
        yield event.plain_result(text)

    @filter.llm_tool(name="time_divination")
    async def tool_time_gua(self, event: AstrMessageEvent, question: str) -> MessageEventResult:
        '''按当前时间起卦（梅花易数时间起卦法）。当用户要求按时间、时辰起卦时调用。调用后你会收到完整卦象数据，请你务必先展示卦象，然后用你自己的人设风格对卦象进行详细解读（200-400字），包括本卦变卦含义、体用关系分析、对所问之事的建议，最后提醒这是娱乐性质的占卜。
